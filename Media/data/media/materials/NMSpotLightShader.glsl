...
void main()
{
	...
	float LDotN = clamp(dot( normalize(vec3(normal.x, normal.y, normal.z)), normalize(light_direction_ts)), 0.0, 1.0);
	float spot_effect = spotEffect(light_direction_os, light_central_ray);
