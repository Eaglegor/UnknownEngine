	...
	namespace ASBind
	{
		...
		class AssignOperatorRegistrator
		{
			public:
				...
				{
					return subsystem->getScriptEngine()->RegisterObjectMethod(FormattedTypeName<T>()(subsystem).c_str(), format_signature(opr, "opAssign", subsystem), asFUNCTION(opr), asCALL_CDECL_OBJFIRST);
				}

				static bool bind(T& (T::*opr)(const T&), Behavior::AngelScriptSubsystem *subsystem)
				{
					return subsystem->getScriptEngine()->RegisterObjectMethod(FormattedTypeName<T>()(subsystem).c_str(), format_member_signature(opr, "opAssign", subsystem), methodRef(opr), asCALL_THISCALL);
				}
