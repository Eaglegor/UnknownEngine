...
#define LOG_ERROR(logger, message) if(logger) logger->log( Core::SimpleLogRecord(Core::LogSeverity::ERROR2, std::string(message) ERROR_LINE_NUMBER ) )
...
namespace boost
{
	...
	UnknownEngine::Core::LogSeverity lexical_cast<UnknownEngine::Core::LogSeverity, std::string>(const std::string &value)
	{
		...
		if(converted_value=="error") return UnknownEngine::Core::LogSeverity::ERROR2;
