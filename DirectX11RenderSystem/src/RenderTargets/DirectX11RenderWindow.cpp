...
namespace UnknownEngine
{
	...
	namespace Graphics
	{
		...
		{
			DirectX11RenderWindow* render_window = reinterpret_cast<DirectX11RenderWindow*>( GetWindowLong( hwnd, GWLP_USERDATA ) );
		...
		DirectX11RenderWindow::DirectX11RenderWindow ( const RenderWindowDesc &desc, const DirectX11RenderSystem* render_system )
				: render_system( render_system ), window_handle( nullptr )
		{
			createWindow( desc );
			showWindow();
		}
		...
		WNDCLASSEX DirectX11RenderWindow::createWindowClass ( const RenderWindowDesc& desc ) const
		{
		...
		LRESULT DirectX11RenderWindow::MessageHandler ( HWND window_handler, UINT umsg, WPARAM wparam, LPARAM lparam )
		{
			return DefWindowProc( window_handler, umsg, wparam, lparam );
		}

		void DirectX11RenderWindow::setFullScreen ( bool value )
		{
			if ( value ) ChangeDisplaySettings( &screen_mode, CDS_FULLSCREEN );
			else ChangeDisplaySettings( NULL, 0 );
		}

		void DirectX11RenderWindow::initScreenMode ()
		{
		...
		DirectX11RenderWindow::WindowPosition DirectX11RenderWindow::getWindowCenteredPosition ( const size_t window_width, const size_t window_height )
		{
			size_t screen_width = GetSystemMetrics( SM_CXSCREEN );
			size_t screen_height = GetSystemMetrics( SM_CYSCREEN );
		...
		void DirectX11RenderWindow::createWindow ( const RenderWindowDesc& desc )
		{
		...
		void DirectX11RenderWindow::showWindow ()
		{
