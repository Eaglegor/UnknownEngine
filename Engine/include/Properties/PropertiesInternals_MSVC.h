...
#define PROPERTIES_SEPARATE_MAP_GETTER_OPTIONAL(type) template<> boost::optional<const type &> PropertiesTree<K>::get_optional (const K &name ) const  throw(PropertyNotFoundException) { const std::unordered_map<K, type>& values = getAllOfType<type>(); const auto found = values.find( name ); if ( found == values.end() ) return boost::optional<const type&>(); return boost::optional<const type&>(found->second); }
